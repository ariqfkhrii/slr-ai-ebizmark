...
function countRetrieved(filteredArticles: FilteredArticleSummary[]) {
  return filteredArticles.filter((article) => Boolean(article.retrieved))
    .length;
}
...
function countReportsExcludedAfterRetrieval(
  filteredArticles: FilteredArticleSummary[],
) {
  return Math.max(
    0,
    countRetrieved(filteredArticles) - countFinalIncluded(filteredArticles),
  );
}
...
function countFinalIncluded(filteredArticles: FilteredArticleSummary[]) {
  return filteredArticles.filter(
    (article) =>
      ...
      Boolean(article.retrieved) &&
      article.raw_article !== null,
  ).length;
}
...
const mainBoxes: BoxTemplate[] = [
  ...
  },
  {
    ...
    subtitle: (filteredArticles) =>
      `n = ${countSoughtForRetrieval(filteredArticles)}`,
    fill: '#ffffff',
    stroke: '#0f172a',
  },
  {
  ...
  },
  {
    ...
    subtitle: (filteredArticles) =>
      `n = ${countFinalIncluded(filteredArticles)}`,
    fill: '#dcfce7',
    stroke: '#16a34a',
  },
];
...
const sideBoxes: BoxTemplate[] = [
  ...
  },
  {
    ...
    subtitle: (filteredArticles) =>
      `n = ${countSoughtForRetrieval(filteredArticles) - countRetrieved(filteredArticles)}`,
    fill: '#fee2e2',
    stroke: '#ef4444',
  },
];

export default function AutoReportingFlowDiagram({
  filteredArticles,
}: AutoReportingFlowDiagramProps) {
    ...
    <Card variant="outlined">
      <CardContent>
        ...
        >
          {[
            {
              label: 'Record dari database',
              value: filteredArticles.length,
              color: '#dbeafe',
            },
            {
              label: 'Record diinklusi',
              value: countIncluded(filteredArticles),
              color: '#dcfce7',
            },
            {
              label: 'Record diekslusi',
              value: countExcluded(filteredArticles),
              color: '#fee2e2',
            },
            {
              label: 'Record akhir yang digunakan',
              value: countRetrieved(filteredArticles),
              color: '#fef9c3',
            },
          ].map((item) => (
            <Box
          ...
          >
            ...
            >
              ...
              <text x={32} y={60} fontSize="12" fill="#64748b">
                Ringkasan seleksi artikel dengan tahap identifikasi,
                penyaringan, pengambilan, evaluasi, dan inklusi.
              </text>
              ...
              <text
                ...
                textAnchor="middle"
              >
                Jumlah pada setiap tahapan akan diperbarui secara otomatis
                mengikuti perubahan data pada tahap sebelumnya.
              </text>
            </svg>
          </Box>
        </Box>
